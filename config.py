...
def get_openrk_object_name():
  """Returns the name of the main openrk shared library object"""
  ...
  if ext_suffix is None:
    ...
    filename = '_openrk.{0}.so'.format(ext_suffix)
...
def get_openrk():
  ...
  return get_shared_object_path() + '/' + get_openrk_object_name()
...
class configuration:
  ...
  if (get_platform()[:6] == 'macosx'):
    linker_flags['gcc'] = ['-fopenmp', '-dynamiclib', '-lpython2.7',
                           '-Wl,-install_name,' + get_openrk_object_name()]
  else:
